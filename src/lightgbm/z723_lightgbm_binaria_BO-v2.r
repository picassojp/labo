...
hs <- makeParamSet( 
         makeNumericParam("learning_rate",    lower=    0.005, upper=    0.3), #mientras más bajo debería ser mejor, el 0.3 puede ser muy alto
         makeNumericParam("feature_fraction", lower=    0.2  , upper=    1.0), 
         makeIntegerParam("min_data_in_leaf", lower=    0L   , upper=  2000L), #revisar estos valores de los experimentos de la competencia anterior
         makeIntegerParam("num_leaves",       lower=   16L   , upper=  1024L), #revisar estos valores de los experimentos de la competencia anterior
         makeIntegerParam("envios",           lower= 5000L   , upper= 15000L) #este parámetro hay que verlo en comparación con el leaderboard público
         
         #algunos parámetros para tener en cuenta en la experimentación
...
EstimarGanancia_lightgbm  <- function( x )
{
  ...
  param_variable  <- list(  early_stopping_rounds= as.integer(50 + 5/x$learning_rate) ) #el early stop va a depender del learning rate
...
#los resultados de la OB fueron muy parecidos a los del exp. anterior HT7232.
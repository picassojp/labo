...
dtrain  <- lgb.Dataset(data   = data.matrix(marzo),
                       label  = clase_binaria,
                       # Truco jedi!
                       #para que el lgbm calcule la ganancia
...
model_lgbm_cv <- lgb.cv(
    ...
    param = list(
        ...
        ##ESTE PARAMETRO NO TOCARLO AL MENOS AL PRINCIPIO
        max_bin = 31,

...
# Poner sus semillas (pertenecen a AD)
semillas <- c(732497,
              681979,
              281887,
              936659,
...
PARAM  <- list()
PARAM$experimento <- "TS9320AD6"

PARAM$exp_input  <- "FE9250AD0"

PARAM$future       <- c( 202107 )

PARAM$final_train  <- c(201908,201909,201910,201911,201912, 202001, 202002, 202003, 202011, 202012, 202101, 202102, 202103)

PARAM$train$training     <- c( 201908,201909,201910,201911,201912, 202001, 202002, 202003, 202011, 202012, 202101, 202102, 202103)
...
dataset_input  <- paste0( "./exp/colectivos/", PARAM$exp_input, "/dataset.csv.gz" )
dataset  <- fread( dataset_input )


#creo la carpeta donde va el experimento
dir.create( paste0( "./exp/colectivos/", PARAM$experimento, "/"), showWarnings = FALSE )
setwd(paste0( "./exp/colectivos/", PARAM$experimento, "/"))   #Establezco el Working Directory DEL EXPERIMENTO
